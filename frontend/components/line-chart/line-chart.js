...
Component({
  ...
  lifetimes: {
    attached() {
      ...
      this.setData({ canvasId: 'linechart-' + Date.now() + '-' + Math.floor(Math.random() * 1000) })
    },
    ready() {
      this.drawChart()
    }
  ...
  methods: {
    ...
    _getTokenColor(name) {
      ...
      const palette = {
        light: { line: '#10B981', lineSoft: 'rgba(16,185,129,0.10)', text: '#9CA3AF', grid: '#F4F1ED', dot: '#FFFFFF' },
        dark:  { line: '#34D399', lineSoft: 'rgba(52,211,153,0.15)', text: '#787673', grid: '#2A2826', dot: '#161513' }
      }[dark ? 'dark' : 'light']
      return palette[name]
    },
